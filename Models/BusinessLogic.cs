    ...
    public class BusinessLogic
    {
        ...
        public static bool IsInWorkingHours(DateTime start, DateTime end)
        {
            ...
            if (holidays.Count()> 0)
            {
                ...
                {
                    if(start.Date.Equals(x.Date))
                    return false;
                }
            }
            if (db.Holidays.ToList().Equals(start.Day))
            {
                return false;
            }
        ...
        public static bool IsInWorkingHours(TimeBlock block)
        {
            ...
            //foreach (var i in db.Holidays.ToList())
            //    dt.Add(i.day);
            if (holidays.Count() > 0)
            {
                ...
                foreach (var x in holidays)
                {
                    if (block.Start.Date.Equals(x.Date))
                        return false;
                }
            }
            if (db.Holidays.ToList().Equals(block.Start.Day))
            {
                return false;
            }
